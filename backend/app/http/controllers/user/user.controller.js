...
class userAuthController extends Controller {
	...
	// Test endpoint for creating users with username/password
	async createTestUser(req, res) {
		const { name, password, phoneNumber = '09123456789' } = req.body
		...
		if (existingUser) {
			return res.status(HttpStatus.BAD_REQUEST).json({
				...
				message: 'کاربر با این مشخصات قبلاً وجود دارد',
			})
		}
		...
		// Generate and set tokens using existing functions
		await setAccessToken(res, user)
		await setRefreshToken(res, user)
